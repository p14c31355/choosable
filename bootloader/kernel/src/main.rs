...
fn inl(port: u16) -> u32 {
    let v: u32;
    unsafe { core::arch::asm!("in eax, dx", out("eax") v, in("dx") port) };
    v
}
...
fn parse_exfat_vbr(part_start_lba: u32) -> Option<ExfatInfo> {
    let mut buf = [0u8; 512];
    if !ata_read_sector(part_start_lba, &mut buf) {
        return None;
    }
...
fn read_fat_entry(info: &ExfatInfo, cluster: u32) -> u32 {
    ...
    let mut buf = [0u8; 512];
    if !ata_read_sector(fat_sector as u32, &mut buf) {
        return 0xFFFFFFFF;
    }
    u32::from_le_bytes([
        buf[fat_offset],
...
fn scan_directory(
    ...
    'outer: loop {
        for sec in 0..sectors_per_cluster {
            ...
            if !read_cluster_sector(info, cluster, sec, &mut sector) {
                return;
            }
...
fn show_menu(files: &[DirEntry], count: usize, part_lba: u32, info: &ExfatInfo) -> ! {
    ...
    vga_print(
        prompt_row,
        1,
        b"Enter number to boot (or 'r' to refresh): ",
        0x07,
    );
...
fn boot_iso(file: &DirEntry, _part_lba: u32, info: &ExfatInfo) -> ! {
    ...
    if &boot_rec[1..6] != b"CD001" {
        vga_print(
            4,
            2,
            b"Invalid Boot Record identifier (expected CD001).",
            0x0C,
        );
        kbd_wait_key();
        show_menu(&[], 0, 0, info);
    ...
    let catalog_iso_lba = u32::from_le_bytes([
        boot_rec[0x47],
        boot_rec[0x48],
        boot_rec[0x49],
        boot_rec[0x4A],
