...
extern "efiapi" fn efi_main(
    ...
    if unsafe { (bs.handle_protocol)(disk_handle, &BLOCK_IO_PROTOCOL_GUID, &mut bio as *mut _ as _) } != EFI_SUCCESS || bio.is_null() {
        die(st, b"ERROR: No Block I/O on disk.\r\n\0");
    }
    let bio_ptr = bio;
    let bio_ref = unsafe { &*bio };
    let mid = if !bio_ref.media.is_null() { unsafe { (*bio_ref.media).mid } } else { 0 };
    ...
    if unsafe { (bio_ref.read_blocks)(bio_ptr, mid, part1_lba, 512, vbr.as_mut_ptr() as _) } != EFI_SUCCESS {
    ...
    unsafe { ROOT_CLUSTER = root_cluster; }

    // Scan root directory for .iso files
    let mut iso_count: usize = 0;
    let mut iso_files: [IsoEntry; 64] = unsafe { core::mem::zeroed() };
    scan_exfat_dir(bio_ref, bio_ptr, mid, root_cluster, sec_per_cluster, fat_start, fat_len, heap_start,
                   &mut iso_files, &mut iso_count);

    // Show menu
    show_menu(st, &iso_files, iso_count, part1_lba, sec_per_cluster, fat_start, fat_len, heap_start, bio_ref, bio_ptr, mid);
    halt_or_reboot(st);
}
...
#[derive(Clone, Copy)]
struct IsoEntry { name: [u8; 256], name_len: usize, start_cluster: u32, file_size: u64, file_size_bytes: u64 }
...
fn scan_exfat_dir(
    ...
    root_cluster: u32, spc: u32, fat_start: u64, fat_len: u64, heap_start: u64,
    files: &mut [IsoEntry; 64], count: &mut usize,
) {
    ...
    'outer: loop {
        for si in 0..spc {
            ...
            if unsafe { (bio.read_blocks)(bio_ptr, mid, lba, 512, buf.as_mut_ptr() as _) } != EFI_SUCCESS { return; }
            ...
            while pos < n_entries {
                ...
                if etype == 0x85 { // File entry
                    ...
                    if entries[stream_off] != 0xC0 { pos += total_ents; continue; }
                    let start_cl = u32::from_le_bytes([entries[stream_off+20],entries[stream_off+21],entries[stream_off+22],entries[stream_off+23]]);
                    let fsize = u64::from_le_bytes([
                        entries[stream_off+24],entries[stream_off+25],entries[stream_off+26],entries[stream_off+27],
                        entries[stream_off+28],entries[stream_off+29],entries[stream_off+30],entries[stream_off+31],
                    ]);
                    ...
                    if name_pos >= 4 && name_buf[name_pos-4..name_pos].eq_ignore_ascii_case(b".iso") && *count < 64 {
        ...
        if unsafe { (bio.read_blocks)(bio_ptr, mid, fat_entry_lba, 512, fat_buf.as_mut_ptr() as _) } != EFI_SUCCESS { return; }
        let next = u32::from_le_bytes([fat_buf[fat_entry_off],fat_buf[fat_entry_off+1],fat_buf[fat_entry_off+2],fat_buf[fat_entry_off+3]]);
...
fn format_u64_buf(v: u64) -> ([u8; 20], usize) {
    ...
    let mut pos = 20;
    let mut n = v;
    while n > 0 && pos > 0 { pos -= 1; buf[pos] = (n % 10) as u8 + b'0'; n /= 10; }
    (buf, 20 - pos)
}

fn show_menu(
    st: &mut SystemTable,
    files: &[IsoEntry; 64], count: usize,
    part1_lba: u64, spc: u32, fat_start: u64, fat_len: u64, heap_start: u64,
    bio: &BlockIoProtocol, bio_ptr: *mut BlockIoProtocol, mid: u32,
) {
    if count == 0 {
    ...
    if !st.con_in.is_null() {
        ...
        loop {
            ...
            let ch = if k.uc >= 0x20 && k.uc < 0x7F { k.uc as u8 } else { match k.sc { 0x1C=>b'\n', 0x13=>b'r', 0x1F=>b'R', _=>0x00 } };
            if (b'1'..=b'9').contains(&ch) {
                let idx = (ch - b'1') as usize;
                if idx < count {
                    boot_iso(st, files, idx, part1_lba, spc, fat_start, fat_len, heap_start, bio, bio_ptr, mid);
                }
            } else if (b'0'..=b'9').contains(&ch) {
                let idx = (ch - b'0') as usize;
                if idx == 1 && count >= 10 {
                    if count >= 10 { boot_iso(st, files, idx + 9, part1_lba, spc, fat_start, fat_len, heap_start, bio, bio_ptr, mid); }
                }
            } else if ch == b'r' || ch == b'R' {
                print_raw(st, b"\r\nRe-scanning...\r\n\0");
                let mut new_files: [IsoEntry; 64] = unsafe { core::mem::zeroed() };
                let mut new_count: usize = 0;
                scan_exfat_dir(bio, bio_ptr, mid, root_cluster_from_first_scan(), spc, fat_start, fat_len, heap_start, &mut new_files, &mut new_count);
                show_menu(st, &new_files, new_count, part1_lba, spc, fat_start, fat_len, heap_start, bio, bio_ptr, mid);
                return;
            }
        }
    }
...
fn root_cluster_from_first_scan() -> u32 { unsafe { ROOT_CLUSTER } }

fn boot_iso(
    st: &mut SystemTable,
    files: &[IsoEntry; 64], idx: usize,
    part1_lba: u64, spc: u32, fat_start: u64, fat_len: u64, heap_start: u64,
    bio: &BlockIoProtocol, bio_ptr: *mut BlockIoProtocol, mid: u32,
) -> ! {
    print_raw(st, b"\r\nBooting ISO...\r\n\0");
    ...
    if unsafe { (bio.read_blocks)(bio_ptr, mid, boot_rec_lba, 512, brec.as_mut_ptr() as _) } != EFI_SUCCESS {
    ...
    if unsafe { (bio.read_blocks)(bio_ptr, mid, catalog_lba, 512, cat.as_mut_ptr() as _) } != EFI_SUCCESS {
    ...
    for i in 0..16 {
        ...
        if cat[off] == 0x88 || cat[off] == 0x90 {
            ...
            boot_image_lba = iso_lba + u32::from_le_bytes([cat[off+8],cat[off+9],cat[off+10],cat[off+11]]) as u64 * 4;
            found = true;
            break;
        }
    }
    if !found { print_raw(st, b"No bootable entry in catalog.\r\n\0"); halt_or_reboot(st); }

    let bs = unsafe { &mut *st.boot_services };
    let mut pages: u64 = 0x100000;
    let status = unsafe { (bs.allocate_pages)(AllocateType::AllocateAddress, MemoryType::EfiLoaderData, 1, &mut pages) };
    if status != EFI_SUCCESS {
        pages = 0;
        let status = unsafe { (bs.allocate_pages)(AllocateType::AllocateAnyPages, MemoryType::EfiLoaderData, 1, &mut pages) };
        if status != EFI_SUCCESS { print_raw(st, b"AllocatePages failed.\r\n\0"); halt_or_reboot(st); }
    }
    let dest = pages as *mut u8;
    let mut sector_buf: [u8; 512] = [0; 512];
    for s in 0..boot_sector_count {
        if unsafe { (bio.read_blocks)(bio_ptr, mid, boot_image_lba + s as u64, 512, sector_buf.as_mut_ptr() as _) } != EFI_SUCCESS {
            print_raw(st, b"Failed to read boot image.\r\n\0");
            halt_or_reboot(st);
        }
        for (j, &b) in sector_buf.iter().enumerate() {
            unsafe { *dest.add(s as usize * 512 + j) = b; }
        }
    }
...
fn find_disk_handle(bs: &mut BootServices, image_handle: *mut core::ffi::c_void) -> Option<*mut core::ffi::c_void> {
    ...
    if unsafe { (bs.handle_protocol)(image_handle, &LOADED_IMAGE_PROTOCOL_GUID, &mut lip as *mut _ as _) } == EFI_SUCCESS && !lip.is_null() {
        ...
        if !dh.is_null() { return Some(dh); }
    }
    // Fallback: locate all Block I/O handles
    let mut num: usize = 0;
    let mut buf: *mut *mut core::ffi::c_void = core::ptr::null_mut();
    if unsafe { (bs.locate_handle_buffer)(LocateSearchType::ByProtocol, &BLOCK_IO_PROTOCOL_GUID, core::ptr::null_mut(), &mut num, &mut buf) } == EFI_SUCCESS && !buf.is_null() && num > 0 {
...
fn print_raw(st: &mut SystemTable, s: &[u8]) { if !st.con_out.is_null() { prints(unsafe { &mut *(st.con_out as *mut SimpleTextOutput) }, s); } }
...
#[repr(C)] struct SystemTable {
    hdr: TableHeader, fw: *const u16, rev: u32, pad_st0: u32,
    cih: *mut core::ffi::c_void, con_in: *mut core::ffi::c_void,
    coh: *mut core::ffi::c_void, con_out: *mut core::ffi::c_void,
    seh: *mut core::ffi::c_void, stderr: *mut core::ffi::c_void,
    runtime_services: *mut RuntimeServices, boot_services: *mut BootServices,
    nte: usize, ct: *mut core::ffi::c_void,
}
#[repr(C)] struct TableHeader { sig: u64, rev: u32, hsz: u32, crc32: u32, reserved: u32 }
...
#[repr(C)] struct BootServices {
    ...
    handle_protocol: unsafe extern "efiapi" fn(*mut core::ffi::c_void, *const Guid, *mut *mut core::ffi::c_void) -> usize, // 0x98
    ...
    locate_handle_buffer: unsafe extern "efiapi" fn(LocateSearchType, *const Guid, *mut core::ffi::c_void, *mut usize, *mut *mut *mut core::ffi::c_void) -> usize, // 0x138
...
#[repr(C)] struct RuntimeServices {
    hdr: TableHeader,
    rs_18: *mut core::ffi::c_void, rs_20: *mut core::ffi::c_void, rs_28: *mut core::ffi::c_void, rs_30: *mut core::ffi::c_void,
    rs_38: *mut core::ffi::c_void, rs_40: *mut core::ffi::c_void, rs_48: *mut core::ffi::c_void, rs_50: *mut core::ffi::c_void,
    rs_58: *mut core::ffi::c_void, rs_60: *mut core::ffi::c_void,
    reset_system: unsafe extern "efiapi" fn(ResetType, usize, usize, *mut core::ffi::c_void) -> !,
}
#[repr(C)] struct BlockIoProtocol {
    bio_rev: u64, media: *mut BlockIoMedia, bio_rst: *mut core::ffi::c_void,
    read_blocks: unsafe extern "efiapi" fn(*mut Self, u32, u64, usize, *mut core::ffi::c_void) -> usize,
    bio_w: *mut core::ffi::c_void, bio_f: *mut core::ffi::c_void,
}
#[repr(C)] struct BlockIoMedia { mid: u32, bim_rm: bool, bim_mp: bool, bim_lp: bool, bim_ro: bool, bim_wc: bool, bim_bs: u32, bim_ia: u32, bim_lb: u64 }
#[repr(C)] struct Guid { d1: u32, d2: u16, d3: u16, d4: [u8; 8] }
#[repr(C)] struct LoadedImageProtocol { _rev: u32, _p: *mut core::ffi::c_void, _st: *mut core::ffi::c_void, device_handle: *mut core::ffi::c_void }
#[derive(Clone, Copy)] #[repr(u32)] enum ResetType { ResetCold = 0 }
#[derive(Clone, Copy)] #[repr(u32)] enum LocateSearchType { ByProtocol = 2 }
#[derive(Clone, Copy)] #[repr(u32)] enum AllocateType { AllocateAnyPages = 0, AllocateAddress = 2 }
#[derive(Clone, Copy)] #[repr(u32)] enum MemoryType { EfiLoaderData = 1 }
