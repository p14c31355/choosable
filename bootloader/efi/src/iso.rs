...
/// Return the name length used by GRUB for an ISO9660 directory record.
///
/// Some images generated by libisoburn store extensionless names with a
/// trailing dot before the version suffix (for example `INITRD.;1`). GRUB
/// exposes that entry as `initrd`, so remove both the `;1` suffix and the
/// compatibility dot before comparing names.
fn iso_effective_name_len(record_name: &[u8]) -> usize {
    let mut len = record_name.len();
    if len >= 2 && record_name[len - 2] == b';' {
        len -= 2;
    }
    while len > 0 && record_name[len - 1] == b'.' {
        len -= 1;
    }
    len
}
...
fn find_in_dir_with_loc(
    ...
    for s in 0..total_sectors {
        ...
        while offset + 34 <= 2048 && offset < (dir_size as usize).saturating_sub(s as usize * 2048) {
            ...
            let effective_len = iso_effective_name_len(&scratch[name_offset..name_offset + name_len]);
...
fn find_first_file_in_dir(
    ...
    for s in 0..total_sectors {
        ...
        while offset + 34 <= 2048 && offset < (dir_size as usize).saturating_sub(s as usize * 2048) {
            ...
            if !is_dot && !is_dir {
                let eff_len = iso_effective_name_len(&scratch[name_offset..name_offset + name_len]);
                if eff_len > 15 {
                    offset += record_len;
                    continue;
                }
...
#[cfg(test)]
mod menu_tests {
    use super::{iso_effective_name_len, parse_menu_number};

    #[test]
    fn normalizes_extensionless_iso9660_names() {
        assert_eq!(iso_effective_name_len(b"INITRD.;1"), 6);
        assert_eq!(iso_effective_name_len(b"VMLINUZ;1"), 7);
        assert_eq!(iso_effective_name_len(b"GRUB.CFG;1"), 8);
    }
